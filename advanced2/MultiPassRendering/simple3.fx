...
float g_occlusionDarkenStrength = 1.2f;
float g_sampleRadius = 1.0f;
float g_distanceFalloffStrength = 0.8f;
float g_targetMatchStrength = 0.7f;
float g_depthCompareBias = 0.03f;
float g_depthCompareFalloff = 7.0f;
float4x4 g_matProj;
                  ...
                  out float4 outColor : COLOR)
{
    ...
    float3 tangentInput = tex2D(tangentSampler, baseUV).rgb * 2.0 - 1.0;
    ...
    float3 baseViewPos = ReconstructViewPosition(baseUV, depth);
    float occlusion = 0.0;
    float occlusionWeight = 0.0;

    for (int i = 0; i < 64; ++i)
    {
        float noise = frac(sin(dot(baseUV + float2(i * 0.123, i * 0.371),
                                   float2(12.9898, 78.233))) * 43758.5453);
        float angleNoise = frac(sin(dot(baseUV + float2(i * 0.719, i * 0.183),
        ...
        float sampleDistance = lerp(0.1, g_sampleRadius, noise * noise);
            ...
            sampleUV.y >= 0.0 && sampleUV.y <= 1.0)
        {
            ...
            float horizonWeight = saturate(dot(sampleDir, SafeNormalize(deltaToHit, sampleDir)));
            float sampleNormalWeight = saturate(1.0 - max(dot(normal, sampleNormal), 0.0));

            float sampleWeight = distanceWeight * targetMatchWeight;
            float occlusionSample = sampleWeight * depthWeight * horizonWeight * sampleNormalWeight;

            occlusion += occlusionSample;
            occlusionWeight += sampleWeight;
        }
    }

    float ao = 1.0 - saturate((occlusion / max(occlusionWeight, 0.0001)) * g_occlusionDarkenStrength);
